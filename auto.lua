...
---
## 🖱️ Скрипт для клика раз в 15 минут

Этот новый блок кода добавляет функционал **автоматического клика** в центр экрана каждые **900 секунд (15 минут)**.

```lua
local function simulateClick()
    -- Получаем текущий размер экрана для расчета центра
    ...
    -- MouseButton1Down
    UserInputService:SimulateMouseClick(centerX, centerY, Enum.UserInputType.MouseButton1, Enum.UserInputState.Begin)
    task.wait(0.05) -- Небольшая задержка для имитации "нажатия"
    -- MouseButton1Up
    UserInputService:SimulateMouseClick(centerX, centerY, Enum.UserInputType.MouseButton1, Enum.UserInputState.End)
    
    print("✅ Автоматический клик выполнен в центре экрана.")
end

local function startAutoClicker()
    local CLICK_INTERVAL_SECONDS = 900 -- 15 минут * 60 секунд = 900 секунд
    
    while true do
        task.wait(CLICK_INTERVAL_SECONDS) -- Ждем 15 минут
        simulateClick()
    end
end

-- Запускаем авто-кликер в отдельном потоке, чтобы он не блокировал основной скрипт
task.spawn(startAutoClicker)

print("Автоматический кликер запущен. Клик будет выполняться каждые 15 минут.")
